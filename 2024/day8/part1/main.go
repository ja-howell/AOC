...
func main() {
	...
	antinodes := findAntinodes(antennas)
...
func findAntinodes(antennas map[byte][]Cell) map[Cell]struct{} {
	...
	for _, antenna := range antennas {
		for i := 0; i < len(antenna); i++ {
			for j := i + 1; j < len(antenna); j++ {
				...
				antinodes[getAntinode(antenna[i], dr, dc)] = struct{}{}
				antinodes[getAntinode(antenna[j], -dr, -dc)] = struct{}{}
			}
		}
	}
	return antinodes
